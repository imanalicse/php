<?php
namespace AttendeeImport\Controller\Admin;

use Cake\Core\Configure;
use Cake\Datasource\ConnectionManager;
use Cake\Event\EventInterface;
use AttendeeImport\Controller\AppController;
use Cake\I18n\FrozenTime;

class AttendeeImportController extends AppController {
    ...
    public function beforeFilter(EventInterface $event)
    {
        ...
        /*
        $this->Security->setConfig('unlockedActions', ['import', 'getPopupInitBody', 'importFile',
        'fieldMapping', 'saveFieldMapping', 'paging', 'saveImportData']);
        */
    }
    ...
    protected function excludeDbFields() {
    ...
    public function fieldMapping() {
        ...
        $this->set(compact('import_students', 'db_field_names', 'required_fields', 'mapped_data', 'db_field_aliasing'));
    }

    protected function getImportedSessionData() {
        $import_students = $this->session->read('import_students');
    ...
    private function getStudentFieldNames() {
        ...
        $prefix = $this->getComponent('CommonFunction')->getDatabaseTablePrefix("events");
        $sql = "DESCRIBE ".$prefix."shared_s3v3_org_events;";
        $table_schema = $connection->execute($sql)->fetchAll('assoc');
        $db_field_names = array_column($table_schema, 'Field');
        $db_field_names = array_diff($db_field_names, $this->excludeDbFields());
    ...
    private function getKeyValueMappedData() {
        $importMapperModel = $this->getDbTable("AttendeeImport.ImportMapper");
        $query = $importMapperModel->find('list', [
                    'keyField' => 'db_field',
                    'valueField' => 'excel_field_label'
                ]);
        $mapped_data = $query->toArray();
        return $mapped_data;
    }
